                    ...
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @foreach ($products as $product)
                                ...
                                <tr>
                                    <td class="px-5 py-4"><div class="flex items-center gap-3">@if($product->primaryMedia)<img src="{{ route('catalog.media.content',$product->primaryMedia->uuid) }}" alt="{{ $product->primaryMedia->alt_text ?? '' }}" loading="lazy" decoding="async" class="h-14 w-14 rounded-lg bg-slate-100 object-contain">@else<div class="h-14 w-14 rounded-lg bg-slate-100"></div>@endif<div><p class="font-semibold text-slate-900">{{ $product->name }}</p><p class="mt-1 font-mono text-xs text-slate-500">{{ $product->slug }}</p><p class="mt-1 text-xs text-slate-500">{{ $product->brand ?: __('No brand') }} · {{ trans_choice(':count category|:count categories', $product->categories_count, ['count' => $product->categories_count]) }} · {{ trans_choice(':count variant|:count variants', $product->variants_count, ['count' => $product->variants_count]) }} · {{ trans_choice(':count image|:count images',$product->product_media_count,['count'=>$product->product_media_count]) }}</p></div></div></td>
