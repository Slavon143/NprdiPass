<x-app-layout>
    ...
    <div class="mx-auto grid max-w-5xl gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[minmax(0,1fr)_20rem] lg:px-8">
        <div class="space-y-6"><section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div class="flex items-center justify-between"><div><h2 class="text-lg font-semibold">{{ __('Variant images') }}</h2><p class="text-sm text-slate-500">{{ trans_choice(':count image|:count images',$variant->media_count,['count'=>$variant->media_count]) }}</p></div><a href="{{ route('catalog.products.variants.media.index',[$product->uuid,$variant->uuid]) }}" class="text-sm font-semibold text-indigo-700">{{ $canManageMedia ? __('Manage images') : __('View images') }}</a></div>@php($displayMedia=$variant->primaryMedia ?? $product->primaryMedia)@if($displayMedia)<img src="{{ route('catalog.media.content',$displayMedia->uuid) }}" alt="{{ $displayMedia->alt_text ?? '' }}" decoding="async" class="mt-4 max-h-80 w-full rounded-xl bg-slate-100 object-contain">@if(!$variant->primaryMedia && $product->primaryMedia)<p class="mt-2 text-xs text-slate-500">{{ __('Showing Product primary image as a visual fallback.') }}</p>@endif @else<div class="mt-4 flex h-48 items-center justify-center rounded-xl bg-slate-100 text-sm text-slate-500">{{ __('No primary image selected') }}</div>@endif</section><section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
