                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach ($variants as $variant)
                            ...
                            <tr>
                                ...
                                <td class="px-5 py-4 text-slate-700"><p><span class="text-slate-400">SKU:</span> {{ $variant->sku ?: '—' }}</p><p><span class="text-slate-400">GTIN:</span> {{ $variant->gtin ?: '—' }}</p><p><span class="text-slate-400">MPN:</span> {{ $variant->mpn ?: '—' }}</p><p><span class="text-slate-400">{{ __('Images') }}:</span> {{ $variant->media_count }}</p></td>
                                ...
                                <td class="px-5 py-4"><div class="flex flex-wrap justify-end gap-2"><a href="{{ route('catalog.products.variants.show', [$product->uuid, $variant->uuid]) }}" class="rounded-lg border border-slate-300 px-3 py-1.5 font-semibold text-slate-700 hover:bg-slate-50">{{ __('View') }}</a><a href="{{ route('catalog.products.variants.media.index', [$product->uuid, $variant->uuid]) }}" class="rounded-lg border border-indigo-300 px-3 py-1.5 font-semibold text-indigo-700 hover:bg-indigo-50">{{ __('Images') }}</a>@if ($canUpdate && $variant->status->value !== 'archived' && $product->status->value !== 'archived')<a href="{{ route('catalog.products.variants.edit', [$product->uuid, $variant->uuid]) }}" class="rounded-lg border border-indigo-300 px-3 py-1.5 font-semibold text-indigo-700 hover:bg-indigo-50">{{ __('Edit') }}</a>@if (! $isDefault)<form method="POST" action="{{ route('catalog.products.variants.set-default', [$product->uuid, $variant->uuid]) }}">@csrf<button type="submit" class="rounded-lg border border-emerald-300 px-3 py-1.5 font-semibold text-emerald-700 hover:bg-emerald-50">{{ __('Set as default') }}</button></form>@endif @endif @if($canArchive && $variant->status->value === 'archived')<form method="POST" action="{{ route('catalog.products.variants.restore', [$product->uuid, $variant->uuid]) }}">@csrf<button type="submit" class="rounded-lg border border-indigo-300 px-3 py-1.5 font-semibold text-indigo-700">{{ __('Restore') }}</button></form>@elseif($canArchive && !$isDefault && $availableVariantCount > 1)<form method="POST" action="{{ route('catalog.products.variants.archive', [$product->uuid, $variant->uuid]) }}">@csrf<button type="submit" class="rounded-lg border border-red-300 px-3 py-1.5 font-semibold text-red-700">{{ __('Archive') }}</button></form>@endif</div></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
